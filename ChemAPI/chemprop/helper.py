...
def predictLogS(mol):
    model_location = "{0}/chemprop/model/{1}".format(
        os.getcwd(), "logS.joblib.pkl")
    try:
        ...
        if "'RandomForestRegressor' object has no attribute 'estimator'" in str(
                e):
            raise Exception(
                "Model compatibility error: The scikit-learn model was "
                "saved with an older version and is incompatible with "
                "the current version. Please retrain the model with "
                "scikit-learn 1.8.0."
            )
