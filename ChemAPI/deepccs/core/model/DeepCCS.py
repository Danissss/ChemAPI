...
class DeepCCSModel(object):
    ...
    def create_model(self):
        ...
        """
        if (
            len(self.smiles_encoder.converter) == 0
            or len(self.adduct_encoder.converter) == 0
        ):
            raise ValueError("Encoders must be fit before creating a model.")
        smile_input_layer = Input(
            shape=(250, len(self.smiles_encoder.converter)), name="smile"
        )
        conv = Conv1D(
            64, kernel_size=4, activation="relu", kernel_initializer="normal"
        )(smile_input_layer)

        previous = conv
        for i in range(6):
            conv = Conv1D(64, kernel_size=4, activation="relu",
                          kernel_initializer="normal")(previous)
            if i == 5:
        ...
        adduct_input_layer = Input(
            shape=(len(self.adduct_encoder.converter),), name="adduct"
        )
        remix_layer = keras.layers.concatenate(
            [flat, adduct_input_layer], axis=-1)
        ...
        # Updated optimizer instantiation for TensorFlow 2.x (lr ->
        # learning_rate)
