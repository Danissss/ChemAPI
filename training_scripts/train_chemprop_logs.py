#!/usr/bin/env python3
"""
Training script for ChemProp LogS (Water Solubility) prediction model
Uses scikit-learn RandomForestRegressor with molecular descriptors from RDKit

Requirements:
- Training data CSV with columns: 'SMILES', 'LogS'
- RDKit for molecular descriptor calculation
- scikit-learn for model training

Usage:
    python train_chemprop_logs.py --data training_data.csv --output ../ChemAPI/chemprop/model/
"""
...
def load_data(data_file):
    ...
    if 'SMILES' not in df.columns or 'LogS' not in df.columns:
        raise ValueError("CSV must contain 'SMILES' and 'LogS' columns")
...
def save_model(model, metrics, output_dir):
    ...
    model_path = os.path.join(output_dir, 'logS.joblib.pkl')
    print(f"\nSaving model to {model_path}...")
    joblib.dump(model, model_path)
    
    # Save metadata
    metadata_path = os.path.join(output_dir, 'logS_metadata.txt')
    with open(metadata_path, 'w') as f:
        f.write(f"LogS Model Training Metadata\n")
...
def main():
    parser = argparse.ArgumentParser(description='Train LogS prediction model')
    ...
    y = df['LogS'].iloc[valid_indices].values
